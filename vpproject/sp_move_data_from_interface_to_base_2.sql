CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_move_data_from_interface_to_base_2`(IN in_report_form_id int,IN in_batch_id int,IN reporting_level varchar(100),IN in_frequency varchar(50),IN in_report_form_group_id int,IN in_week int , IN in_month int ,IN in_bi_month int, IN in_quarter int,IN in_calendar_year int)
BEGIN 
...
LEFT JOIN technical_area AS ta ON de.technical_area_id = ta.technical_area_id INNER JOIN validation_report vr on vr.batch_id=id.batch_id and vr.health_facility_id=id.health_facility_id where vr.batch_id=",in_batch_id," AND vr.status_v='Pass'");
...
INNER JOIN section AS se ON de.section_id = se.section_id
INNER JOIN sub_section AS ss ON de.sub_section_id = ss.sub_section_id
LEFT JOIN technical_area AS ta ON de.technical_area_id = ta.technical_area_id INNER JOIN validation_report vr on vr.batch_id=id.batch_id and vr.parish_id=id.parish_id where vr.batch_id=",in_batch_id," AND vr.status_v='Pass'");
...
INNER JOIN section AS se ON de.section_id = se.section_id
INNER JOIN sub_section AS ss ON de.sub_section_id = ss.sub_section_id
LEFT JOIN technical_area AS ta ON de.technical_area_id = ta.technical_area_id INNER JOIN validation_report vr on vr.batch_id=id.batch_id and vr.district_id=id.district_id where vr.batch_id=",in_batch_id," AND vr.status_v='Pass'");
